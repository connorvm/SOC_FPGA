...
int main()
{
    ...
    uint32_t *base = (uint32_t *) mmap(NULL, LW_SPAN,
        PROT_READ | PROT_WRITE,MAP_SHARED, devmem_fd, LW_BASE);

    uint32_t *led_reg = base + led_offset + 2;
    ...
    uint32_t *hs_led_control = base + led_offset + 0;
    ...
    *hs_led_control = 0;
    ...
    int result = munmap(base, LW_SPAN);
